...
@section('title','បន្ថែមបុគ្គលិក')
@section('page-title','បន្ថែមបុគ្គលិកថ្មី')
...
<x-card>
    <div class="p-6 space-y-4">
        <div class="grid grid-cols-2 gap-4">
            <x-form-input label="លេខកូដ" name="staff_code" :required="true" :value="old('staff_code')" placeholder="ឧ. STF-001"/>
        ...
        <div>
            ...
            <select name="position" required class="block w-full rounded-lg border-slate-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                <option value="office"   {{ old('position','office')=='office'   ? 'selected':'' }}>ការិយាល័យ</option>
                <option value="cleaner"  {{ old('position')=='cleaner'  ? 'selected':'' }}>អ្នកសំអាត</option>
                <option value="security" {{ old('position')=='security' ? 'selected':'' }}>សន្តិសុខ</option>
                <option value="other"    {{ old('position')=='other'    ? 'selected':'' }}>ផ្សេង</option>
            </select>
        </div>
        <div class="grid grid-cols-2 gap-4">
