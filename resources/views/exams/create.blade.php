...
@section('title','បន្ថែមការប្រឡង')
@section('page-title','បន្ថែមការប្រឡងថ្មី')
...
<x-card>
    <div class="p-6 space-y-4">
        <x-form-input label="ឈ្មោះការប្រឡង" name="name" :required="true" :value="old('name')" placeholder="ឧ. ការប្រឡងប្រចាំខែ វិច្ឆិកា"/>
        <div>
            ...
            <select name="exam_type" required class="block w-full rounded-lg border-slate-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                <option value="monthly"  {{ old('exam_type')=='monthly'  ? 'selected':'' }}>ប្រចាំខែ</option>
                <option value="midterm"  {{ old('exam_type')=='midterm'  ? 'selected':'' }}>មធ្យមភាគ</option>
                <option value="final"    {{ old('exam_type')=='final'    ? 'selected':'' }}>ចុងឆមាស</option>
            </select>
        </div>
        <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">ឆ្នាំសិក្សា <span class="text-red-500">*</span></label>
            <select name="academic_year_id" required class="block w-full rounded-lg border-slate-300 text-sm focus:border-indigo-500 focus:ring-indigo-500" id="aySelect">
        ...
        <div class="grid grid-cols-2 gap-4">
            <x-form-input label="ថ្ងៃចាប់ផ្ដើម" name="start_date" type="date" :value="old('start_date')"/>
            <x-form-input label="ថ្ងៃបញ្ចប់"   name="end_date"   type="date" :value="old('end_date')"/>
        </div>
    </div>
